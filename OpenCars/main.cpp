...
int main(int argc, char* argv[])
{
	...
	point1->setLocation(glm::vec3( 2.5, 2,  2.5));
	point2->setLocation(glm::vec3( 2.5, 2, -2.5));
	point3->setLocation(glm::vec3(-2.5, 2,  2.5));
	point4->setLocation(glm::vec3(-2.5, 2, -2.5));
	...
	Box *tramplinG = new Box(glm::vec3(3, 0.5, 3));
	...
	rp3d::BoxShape shapetramplin(Vector3(3, 0.5, 3));
	...
	while (WM_QUIT != msg.message)
	{
		...
		else
		{
			...
			float dampingForce = body->getLinearVelocity().y * damping;
				...
				if (cal.isHit())
				{
					float springForceY =400* (heightSpring - cal.dist);
					float force = springForceY;
					body->applyForce(Vector3(0, force, 0), Vector3(pos.x, pos.y, pos.z));
